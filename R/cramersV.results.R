# cramersV.results
cramersV.results <- function(cormat, sort.by="r", data=NULL, var.name=NULL, na.rm=TRUE) {
  ...
  if (!is.null(data)) {
    ...
    for (i in 1:nrow(result)) {
      ...
      tmp <- cramersV.test( data[,as.character(result$x[i])], 
      ...
      ### CONF. INT NOT IMPLEMENTED FOR CRAMER'S V ###
  ...
  # Sort results into order required by user
